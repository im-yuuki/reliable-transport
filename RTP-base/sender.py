...
def sender(receiver_ip, receiver_port, window_size):
    ...
    start_packet = make_packet(START, 0)
    sock.sendto(start_packet, destination)
    while True:
    ...
    while base_seq <= total_packets:
        ...
        ack_header = receive_ack(sock, POLL_INTERVAL)
        if ack_header is not None and ack_header.seq_num > base_seq:
            base_seq = min(ack_header.seq_num, total_packets + 1)
